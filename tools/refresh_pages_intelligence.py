#!/usr/bin/env python3
"""Refresh reader-visible GHE size, Git contribution metrics, and Work Ledger integration records for IaT Docs Engine.
...
def working_tree_size(root: Path) -> int:
    excluded_parts = {
        ".git", ".template-backup", "_site", "site", ".jekyll-cache", ".sass-cache",
...
def section_labels(root: Path, parent: str | None, commit: str, path: str) -> list[str]:
    if path == "data/document.json":
        return ["Document identity and control metadata"]
    if path == "data/revisions.json":
...
def main() -> int:
    ...
    document_path = root / "data/document.json"
    try:
        document = load_json_object(document_path)
        config_path = root / "data/contribution-config.json"
    ...
    if not args.apply:
        print("DRY RUN: add --apply to write reader data and Work Ledger integration records.")
        return 0

    atomic_write(root / "data/repository.json", json.dumps(repository, indent=2, ensure_ascii=False) + "\n")
    atomic_write(root / "data/contributions.json", json.dumps(contributions, indent=2, ensure_ascii=False) + "\n")
    ...
    print("UPDATED: data/repository.json")
    print("UPDATED: data/contributions.json")
