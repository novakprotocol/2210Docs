...
def _doctor(root: Path) -> dict[str, Any]:
    ...
    for distribution in ("Jinja2", "mistune"):
        try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init.add_argument("--ghe-host", default="va.ghe.com")
    init.add_argument("--allow-canonical", action="store_true")
    init.add_argument("--apply", action="store_true")
    init.add_argument("--json", action="store_true")

    migrate = sub.add_parser("migrate", help="Migrate a v0.07 Jekyll-derived controlled document to IaT Docs Engine")
